...
b = id_block(1); % bloco inicial ; %id_block(2) == numero de blocos a processar

for k=1:id_block(2) % para cada bloco
    current_folder = pwd;
    cd('dados/roi');
    blockfile = sprintf('roi_block%d.mat',b);
    load(blockfile);
    if k==1
        cband = size(Block,2); % conta as bandas selecionadas
    end
    cd(current_folder);        
    fpatterns = sprintf('roi_features%d.mat',b)
    b = b + 1;
    for m=1:cband   % para cada banda        
        ...
        for i=borda+1:size(Block(m).img,1)-borda
            for j=borda+1:size(Block(m).img,2)-borda
                if Block(m).BW(i,j)
                    cont = cont+1;
                    % obter uma secao da Imagem do tamanho do filtro e cujo centro seja o ponto (x,y)
                    ...
                    %signature = apply_Gabor(sec, N, F(1).est,F(1).ori); 
                    % normaliza o vetor para que os valores fiquem entre 0 e 1
                    % (25/04/2006)
                    %signature = signature-min(min(signature));
                    %Block(m).patterns(cont,3:end) = signature/max(max(signature));
                end
            end
        end
...
% aplica os filtros de Gabor na imagem 'tile'
